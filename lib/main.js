...
function fetchPromiseCreator() {
	if (!promiseCreator) {
		promiseCreator = require('./promises-creator');
	}
...
module.exports = {
	...
	activate() {
		...
		[fetchSettings().sPythonPath, fetchSettings().sPythonExecutable, fetchSettings().sConfig, fetchSettings().sCustomToml, fetchSettings().sLintTriggerFlake8, fetchSettings().sLintTriggerMypy, fetchSettings().sLintTriggerPydocstyle, fetchSettings().sLintTriggerPylint].forEach(s => {
