...
const promiseCreator = require('./promises-creator');
...
function fetchErrHandling() {
	if (!errHandling) {
		errHandling = require('./error-handling');
	}
...
function getTomlConfigs(filePath) {
	const configToml = toml.parse(fs.readFileSync(filePath, 'utf-8'));
	return configToml;
}

function getPythonPath(fileDir, projectPath) {
	let pathString = settings.getS(settings.sPythonPath);
	if (settings.getS(settings.sCustomToml)) {
		pathString = getTomlConfigs(adjustPath(settings.getS(settings.sCustomToml), fileDir, projectPath)).PythonPath;
	}

	return adjustPath(pathString, fileDir, projectPath);
}

function getPythonExecutablePath(fileDir, projectPath) {
	let pathString = settings.getS(settings.sPythonExecutable);
	if (settings.getS(settings.sCustomToml)) {
		pathString = getTomlConfigs(adjustPath(settings.getS(settings.sCustomToml), fileDir, projectPath)).PythonExecutable;
	}
...
function getConfigPath(fileDir, projectPath) {
	let pathString = settings.getS(settings.sConfig);
	if (settings.getS(settings.sCustomToml)) {
		pathString = getTomlConfigs(adjustPath(settings.getS(settings.sCustomToml), fileDir, projectPath)).LintConfig;
	}
